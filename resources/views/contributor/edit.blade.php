        ...
        <div style="display:grid;grid-template-columns:2fr 1fr;gap:1.5rem;">
            ...
            <div style="display:flex;flex-direction:column;gap:1.25rem;">
                <div class="glass-card" style="padding:1.5rem;">
                    ...
                    @php $galleryImages = old('existing_gallery_images', is_array($article->images ?? null) ? $article->images : []); @endphp
                    ...
                    <div id="featured-preview-inline" style="display:none;align-items:center;gap:0.75rem;margin-bottom:0.75rem;padding:0.6rem;border:1px solid var(--border-subtle);border-radius:0.65rem;background:rgba(255,255,255,0.02);">
                        ...
                        <div style="font-size:0.75rem;color:var(--text-secondary);">Uploaded to B2 and linked to this article.</div>
                    </div>
                    ...
                    <div style="display:flex;align-items:center;gap:0.75rem;margin-bottom:0.75rem;padding:0.6rem;border:1px solid var(--border-subtle);border-radius:0.65rem;background:rgba(255,255,255,0.02);">
                        <img src="{{ $article->featured_image }}" alt="Featured image" style="width:72px;height:54px;object-fit:cover;border-radius:0.45rem;border:1px solid var(--border-subtle);">
...
(function() {
    ...
    async function handleFeaturedUpload(files) {
        ...
        try {
            const url = await uploadImageToB2(file, 'articles/featured');
            if (!url) return;
            if (featuredInput) {
    ...
    async function handleGalleryUpload(files) {
        ...
        for (const file of entries) {
            try {
                const url = await uploadImageToB2(file, 'articles/gallery');
                if (url) {
